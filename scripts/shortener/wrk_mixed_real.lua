-- 80% codes in Redis (hot), 20% deleted from Redis (cold → async MySQL)
-- Cold codes backfill on first hit, so they gradually warm during the test
...
function init(args)
    local f = io.open("scripts/shortener/codes_mixed_test.txt", "r")
    if not f then
    ...
    f:close()
    print("Loaded " .. count .. " codes (~20% cold, warm up during test)")
end
