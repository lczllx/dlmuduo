...
**云服务器（4c8g 5M 带宽）** — 所有测试均在云服务器上使用 wrk 和 WebBench 进行：

| 环境 | 接口 | 工具 | QPS | CPU利用率/带宽 | 说明 |
|------|------|------|-----|---------------|------|
| 4c8g 云 | /hello | wrk 8线程 1万连接 | **36,775** | 156% | 空载 |
| 4c8g 云 | /hello | wrk 8线程 1万连接 | **33,861** | 141% | **十万长连接下** |
...
**云服务器测试结果（4c8g 5M 带宽）：**
- **1万连接**: QPS 36,775，CPU 156%（多核，8线程）
- **10万连接**: QPS 33,861，CPU 141%
- **性能下降**: QPS 仅下降 **7.9%**，说明 muduo 在 10 万连接下性能保持良好
- **连接数**: 100,000 并发 TCP 连接（单机多 IP 压测）
- **历史数据**: 4c8g 5M 带宽十万连接下 QPS 15,364（不同测试环境）
