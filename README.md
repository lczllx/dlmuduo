...
**云服务器（4c8g 5M 带宽）** — 所有测试均在云服务器上使用 wrk 和 WebBench 进行。4 核机建议 wrk 用 **4 线程**（与核数一致），QPS 更稳定、更高。

| 环境 | 接口 | 工具 | QPS | CPU利用率/带宽 | 说明 |
|------|------|------|-----|---------------|------|
...
| 4c8g 云 | /hello | WebBench **1万** 并发 | **29,799** | - | 短连接，30s（1,787,960 pages/min，893,980 成功 , 0失败） |
...
| 4c8g 云 | /hello | wrk 8线程 1万连接 | 15,364 | 1.47 MB/s | 十万长连接下（历史） |
| 4c8g 云 | /hello | WebBench 1万并发 | 16,720 | 2.33 MB/s | 每请求新建连接 |
...
**QPS 性能：**
- **普通 HTTP（无长连接）**: QPS **40,352**，平均内存 RSS **212MB**，峰值 RSS **394MB**
- **10万/20万长连接下**: QPS **34,027**，平均内存 RSS **880MB**，峰值 RSS **1,045MB**
- **QPS 下降**: 约 **15%**（40,352 → 34,027）
- **内存占用**: 维持 20 万连接时内存增加 **321MB**（394MB → 715MB）；压测时峰值达 **1,045MB**

**内存分析：**
- **维持连接内存**: 约 **321MB**（20 万连接，平均每个连接约 **1.6KB**）
- **压测时内存**: 峰值 **1,045MB**（连接管理 + 请求处理缓冲区）

**连接数**: 100,000～200,000 并发 TCP 连接（单机多 IP 压测）
...
基于自研 Reactor 构建的短链接服务，提供短码生成与 302 重定向跳转。
